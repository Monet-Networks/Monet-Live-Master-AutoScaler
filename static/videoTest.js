...
// the whole session.
//
var server = 'wss://' + window.location.hostname + '/ws';
...
function registerUsername() {
  if ($('#username').length === 0) {
    ...
    if (/[^a-zA-Z0-9]/.test(username)) {
      $('#you').removeClass().addClass('label label-warning').html('Input is not alphanumeric');
...
function subscribeTo(sources) {
        ...
        if (!feedStreams[stream.id].slot) {
          ...
          for (var i = 1; i < 6; i++) {
            if (!feeds[i]) {
              ...
              $('#remote' + slot)
                .removeClass('hide')
                .html(escapeXmlTags(stream.display))
                .show();
              break;
            }
          }
  ...
  janus.attach({
    ...
    onremotetrack: function (track, mid, on) {
      ...
      if ($('#remotevideo' + slot + '-' + mid).length > 0) return;
      if (track.kind === 'audio') {
        ...
        $('#videoremote' + slot).append(
          '<audio class="hide" id="remotevideo' + slot + '-' + mid + '" autoplay playsinline/>'
        );
        ...
        $('#videoremote' + slot).append(
          '<video class="rounded centered" id="remotevideo' + slot + '-' + mid + '" width=100% autoplay playsinline/>'
        );
        $('#videoremote' + slot).append(
        ...
        if (!bitrateTimer[slot]) {
          ...
          bitrateTimer[slot] = setInterval(function () {
            if (!$('#videoremote' + slot + ' video').get(0)) return;
